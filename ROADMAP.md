...
**Implementation status:** the observation, generic coordinate-bounds, and ordinary exact-name anchor-validation slices are complete. GTF/GFF3 feature rows stream into compact sparse summaries; used seqids project into mandatory presence requirements; exhaustive ordinary one-based closed bounds checks project through one anchor-owned `CoordinateBoundsValidationCapability`; unfamiliar names remain unresolved; FASTA supersets remain valid for sparse annotations; and a proven ordinary out-of-bounds feature is a hard Tier-B structural conflict. Potential circular GFF3 bounds are deliberately left unresolved until the dedicated circular-semantics slice. This is the planned internal-review checkpoint before verified annotation binding and GFF3-specific region/circular behavior are built on top.
