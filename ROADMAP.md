...
**Implementation status:** scientific/profile contract, immutable provider snapshot, shared full-anchor target-content resolution, authoritative UCSC canonical/alias name resolution, generic profile binding requirements, resource-local authoritative-name `SequenceBinding` projection, and the first representative VCF end-to-end path are implemented; the first internal scientific/code checkpoint and targeted Slice 4 external review are complete, with one non-blocking regression-coverage gap hardened before broader format integration.
