...
**Implementation status:** observation, generic coordinate-bounds reasoning, ordinary anchor validation, GFF3 sequence-region/provenance handling, verified embedded-FASTA sequence binding, and landmark-aware circular-origin semantics are complete. GTF/GFF3 remain sparse resources; exact names or content-verified `SequenceBinding` values can resolve reference seqids; feature and declared-region coordinates project through one anchor-owned `CoordinateBoundsValidationCapability`; relevant embedded GFF3 FASTA sequences contribute content-derived MD5 identity requirements/capabilities; full-anchor uniqueness prevents scope-manufactured aliases; contradictory embedded content can produce Tier-A identity conflict without replacing the selected FASTA anchor; and circular wrapping is accepted only for an exact logical landmark under the standard single-wrap encoding. Milestone 5 integration/adversarial exit coverage remains before the external review boundary.
