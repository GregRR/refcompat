...
**Implementation status:** core implementation and integration/adversarial exit coverage are complete; external milestone-boundary review remains before Milestone 6. GTF/GFF3 remain sparse resources; exact names or content-verified `SequenceBinding` values can resolve reference seqids; feature and declared-region coordinates project through one anchor-owned `CoordinateBoundsValidationCapability`; relevant embedded GFF3 FASTA sequences contribute content-derived MD5 identity requirements/capabilities; independently established annotation-owned content identities may support conservative GTF/GFF3 binding without becoming intrinsic format claims; full-anchor uniqueness prevents scope-manufactured aliases; contradictory embedded content can produce Tier-A identity conflict without replacing the selected FASTA anchor; and circular wrapping is accepted only for an exact logical landmark under the standard single-wrap encoding.
...
- Apply the GFF3 circular-origin exception only when `Is_circular=true` is carried by the exact logical landmark (`ID == seqid`), the unique landmark begins at coordinate 1, its length matches the resolved anchor, and the extended coordinate uses at most one standard wrap; otherwise preserve unresolved or ordinary conflict/input-invalid states as appropriate. **Implemented.**
- Exercise the complete annotation contract through redistributable integration/adversarial fixtures covering exact sparse GTF/GFF3 coordinates, externally content-verified GTF binding, unresolved naming differences, hard bounds conflicts, sequence-region/circular rules, embedded-content identity conflict, provenance-vs-identity claims, duplicate identity ambiguity, non-model scaffolds, and mixed hard/unresolved problems. **Implemented.**
