...
- explicit BED3–BED9/BED12 layout, first-class BED report identity, and strict native-coordinate streaming inspection (Milestone 9 Slice 3; FASTA reasoning pending);
...
Milestone 9 Slices 1–3 now pin the BED coordinate-compatibility contract,
implement its atomic layout/resource/schema foundation, and add strict
plain/gzip streaming observation in native zero-based half-open coordinates.
BED-to-FASTA reasoning remains pending. Liftover, broad workflow validation,
automatic repair, and complex ecosystem profiles are intentionally deferred.
...
- [`docs/bed-compatibility.md`](docs/bed-compatibility.md) — pinned Milestone 9 explicit-layout, zero-based half-open coordinate, sparse-reasoning, and schema-version boundary;
...
Milestone 9 Slices 1–3 are complete. Standard BED3–BED9/BED12 layouts and
...
The strict streaming inspector preserves zero-based half-open coordinates and
valid empty intervals, summarizes sparse `chrom` usage, validates every declared
standard field plus BED12 blocks, detects gzip by content, and emits no verdict.
Slice 4 will add exhaustive BED-to-FASTA bounds reasoning and generic projection.
