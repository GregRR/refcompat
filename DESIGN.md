...
Milestone 6 adds one second scientifically distinct binding path: an explicit profile may contribute an authoritative sequence-name relationship only when the provider alias resolves uniquely in the complete relevant provider context **and** the provider target sequence is independently content-bound to one sequence in the complete FASTA anchor. The shared `AnchorIdentityResolution` helper now factors the reviewed full-anchor content-match/exhaustive-absence semantics out of resource binding so UCSC target reasoning can reuse them without making provider facts into peer capabilities or reference authority. Provider canonical-name and alias resolution remain separate from this content bridge; Slice 3 deliberately does not yet create a resource-local `SequenceBinding` from those two facts. The resulting relationship remains a naming relationship, not a new content identity for the peer resource. Generic binding vocabulary may therefore still need the smallest authoritative-name evidence trace when Slice 4 projects the relationship end to end, but generic code must not learn UCSC identifiers or infer aliases from strings. The existing content-identity binding path and its invariants remain unchanged.
...
A fully positive UCSC-target relationship requires an independently comparable content identity linking each provider target sequence needed for a mandatory in-scope relationship to exactly one sequence in the complete FASTA anchor. Exact UCSC names do not bypass that rule. An authoritative provider alias may then resolve a resource-local name only when the alias is unique in the complete provider alias context and points to that content-bound target. Scope is applied only after provider-alias uniqueness and full-anchor target uniqueness have been established.

Profile projection should add `RequirementOrigin.PROFILE` requirements or evidence-backed relationships and then reuse existing generic/core presence, length, identity, coordinate-bounds, VCF REF, BAM/CRAM dictionary, finding, and verdict behavior. It must not suppress a core-format requirement to obtain a positive result, and a direct content contradiction always outranks provider support. If the existing generic model cannot retain the provider-alias evidence without pretending that it is peer-owned content identity, introduce the smallest typed relationship extension rather than distorting `SequenceIdentityCapability`.

Provider acquisition is an adapter concern. The same fixed snapshot must yield the same reasoning result with or without network access. Failure to acquire optional/current provider evidence leaves the UCSC-specific relationship unresolved; it is not biological incompatibility. The ordinary quality gate uses frozen redistributable snapshots/fixtures, with any live-UCSC smoke path kept separate.
