...
Milestone 9 Slices 1–2 establish the BED boundary without yet parsing feature
rows: `BedLayout` requires one exact standard layout from BED3 through BED9 or
BED12, `ResourceKind.BED` is first-class, current stable output advances
atomically to exact schema `3.0.0`, provisional output advances to revision 5,
and the first BED known answer records the declared layout as a generic resource
observation. Exact `1.0.0`, `1.1.0`, and `2.0.0` report assets remain retained.
Native zero-based half-open observation begins in Slice 3.
