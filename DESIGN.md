...
- VCF/bgzipped VCF;
- BCF2 (Milestone 8 complete after internal review, external review, provider-error remediation, and targeted confirmation);
- BED3–BED9/BED12 (Milestone 9 Slices 1–3 complete through strict native-coordinate inspection; FASTA reasoning pending);
...
Milestone 7 owns the stable report boundary. The public machine-readable shape must be an explicit RefCompat-owned reporting contract rather than a recursive serialization of internal dataclasses or upstream/provider objects. Report assembly consumes already-derived reasoning outputs, validates their cross-object consistency, and preserves traceability; it does not inspect files again, rebuild constraints, or introduce a second scientific verdict engine. Schema versioning is independent of the package version. M7 froze stable core schema `1.0.0` and then additive schema `1.1.0`, while retaining exact older validators. Milestone 8 Slice 2 advanced the stable serializer to exact schema `2.0.0` because the already-closed `ResourceKind` enum gained `bcf`. Milestone 9 Slice 2 now advances the current serializer to exact schema `3.0.0` because the same closed enum gains `bed`; the body shape otherwise remains the 2.0.0 shape and exact 1.0.0/1.1.0/2.0.0 schemas remain retained. The draft surface advances separately to revision 5. Report-owned observations, including the explicit BED layout, BAM/CRAM dictionary relationship summaries, and UCSC provider/source/profile provenance continue to be projected from already-derived values without leaking implementation objects or creating new verdict logic.
...
The stable JSON Schema was deliberately frozen only after the report model stabilized in Milestone 7. Schema/versioning remains a compatibility boundary; the project does not add validation frameworks merely to mirror internal models.
...
No unresolved tooling choice blocks the next implementation slice. The first stable
machine-readable report schema/versioning commitment was resolved in Milestone 7
(`1.0.0` core and additive `1.1.0`), and M8 Slice 2 advances the current exact
serializer/schema to `2.0.0` for the new closed `bcf` resource-kind value. Remaining decisions are deliberately
milestone-specific and should be resolved when their evidence exists, including
whether later performance or format requirements justify additional parser/storage
dependencies.
...
14. implement Milestone 9 standard BED coordinate compatibility through an explicit BED3–BED9/BED12 layout, native zero-based half-open observations, sparse presence/bounds reasoning, stable report schema `3.0.0`, and reviewable end-to-end slices (Slices 1–3 complete: contract, atomic layout/resource/schema foundation, and strict native-coordinate streaming inspection);
