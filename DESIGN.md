...
- VCF/bgzipped VCF;
- BCF2 (Milestone 8; scoped/profile/reporting parity and internal adversarial review implemented; external milestone review pending);
...
13. implement Milestone 8 native BCF2 support by reusing the existing VCF logical observation/reasoning path, preserving one-based logical POS at the pysam boundary, enforcing declared-vs-detected encoding, and advancing stable report schema to `2.0.0` because `ResourceKind` is a closed enum (scoped/profile/reporting parity and internal adversarial review implemented; external milestone review pending);
