...
Milestone 5's annotation contract is pinned, and the observation, generic coordinate-bounds, and ordinary exact-name anchor-validation slices are complete. GTF/GFF3 are observed through a narrow streaming boundary that preserves native one-based closed coordinates, summarizes sparse used seqids without constructing gene-model hierarchy, preserves raw plus decoded GFF3 seqids, records reference-relevant sequence-region/provenance/`Is_circular=true` facts, recognizes embedded-FASTA boundaries, and supports gzip input without relying on a filename suffix. The generic reasoning layer includes scalable anchor-named `CoordinateBoundsRequirement` values, anchor-owned exhaustive validation capabilities, Tier-B structural evidence, bundle supplemental-capability handling, coordinate-conflict findings, and normal verdict/conflict-core propagation without embedding GTF/GFF3 policy in generic code. Ordinary annotation projection now creates mandatory used-seqid presence requirements and one exhaustive coordinate requirement; exact-name features are checked against scoped FASTA lengths; FASTA supersets remain compatible with sparse annotations; unfamiliar names remain unresolved; and proven ordinary out-of-bounds intervals are hard structural conflicts. Possible circular GFF3 bounds are held unresolved until the dedicated circular-semantics slice. The planned Slices 1–4 internal-review checkpoint is complete. Slice 5 now adds GFF3 `##sequence-region` anchor validation, region-only presence requirements, ordinary feature/region self-consistency enforcement, and explicit tests that provenance claims do not change the contract. Verified annotation sequence binding, embedded-FASTA identity, and landmark-aware circular-origin semantics remain next. Missing candidate evidence remains unresolved unless explicit evidence proves a contradiction, ambiguous sequence identity never manufactures a binding, peer resources do not vote on reference identity, advisory constraints do not veto mandatory compatibility, evidence is never converted into a numeric score, conditions qualify only otherwise-positive verdicts, and failure reporting excludes non-decisive mismatch noise. Analysis status and stable `CompatibilityReport` serialization remain later boundaries. Additional broad corpus collection is not currently justified unless implementation exposes a new unresolved category, a profile requires targeted evidence, or a separate prevalence study becomes a project goal.
