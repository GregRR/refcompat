...
- Begin Milestone 8 implementation: add first-class `ResourceKind.BCF`, require pysam-detected BCF/VCF encoding to match the declared resource kind, reuse the existing VCF logical context/REF observation models with one-based provider-normalized POS preserved, advance the current stable compatibility-report schema to exact `2.0.0` and draft output to revision 4, retain exact `1.0.0`/`1.1.0` schemas unchanged, and pin a minimal BCF known-answer report before RCHECK-050 scientific parity work.
- Pin the Milestone 8 native BCF contract: treat BCF2 as the binary encoding of the existing VCF logical compatibility model, require declared BCF/VCF resource kind to agree with HTSlib-detected encoding, reuse RCHECK-050 context/REF/binding/evidence semantics without BCF-specific verdict logic, preserve one-based logical POS through the pysam boundary, and plan an explicit stable-report schema 2.0.0 because adding `bcf` widens the closed `ResourceKind` enum.
- Close the Milestone 7 external milestone-boundary review: the independent reviewer reproduced the full gate, found no MAJOR issue, and assessed the repository SAFE TO CLOSE M7; harden accepted MINOR findings with general serialized-ID closure and live 1.0.0-core additivity checks across integration reports, portable provenance-locator obligations, canonical human ordering for report-global relationship/profile context, concrete PARTIAL/INVALID_INPUT reporting-pipeline examples, and documentation of intentionally opaque provider trace fields without changing scientific verdicts or stable JSON shape.
- Harden the Milestone 7 exit boundary after adversarial internal review: retain profile-referenced validation and anchor-identity capabilities in stable JSON even for UCSC content-conflict paths, pin that conflict as a schema-valid known answer, prove the additive `1.1.0` core preserves retained `1.0.0` compatible/incompatible payloads, cross-reject exact minor-version schemas, and correct the pre-release `1.1.0` schema description without changing verdict, workflow, or wire-shape semantics.
- Add Milestone 7 representative end-to-end reporting coverage across scoped VCF, BAM/CRAM dictionary relationships, GTF annotation conflicts/unresolved naming, and the UCSC preflight alias path; each representative result now crosses report assembly, exact `1.1.0` schema validation, deterministic JSON/human rendering, and the stable workflow exit policy without adding new scientific reasoning.
- Add the Milestone 7 human/workflow reporting surface: render deterministic plain-text `CompatibilityReport` views without local artifact paths or new scientific conclusions, and pin stable whole-bundle workflow exit codes (`0` positive complete, `1` incompatible, `2` invalid input, `3` indeterminate, `4` partial, `5` pre-report operational failure) while leaving the provisional Milestone 1 CLI diagnostics unchanged.
- Add Milestone 7 report-owned relationship/provenance context and advance the current stable compatibility-report schema additively to `1.1.0`: project BAM/CRAM dictionary membership/naming/order/M5 diagnostics, report-owned resource observations, and UCSC provider/source/profile trace records without re-running scientific reasoning or serializing provider implementation objects; retain exact schema `1.0.0`, and correct its over-escaped refget identity regex as a schema-only erratum so the frozen validator accepts the already-defined `SQ.<32-character>` wire value.
...
- Pin the Milestone 7 stable compatibility-report and workflow contract: keep analysis status separate from scientific verdicts, assemble reports only from existing traceable reasoning outputs, require explicit deterministic RefCompat-owned serialization plus a checked-in versioned schema, surface alignment relationship context without a second verdict, and defer schema/exit-code freezing until an internal API review.
- Close the Milestone 6 external milestone-boundary review: the independent reviewer found no MAJOR scientific or architectural defect and assessed the repository SAFE TO BEGIN M7; add a regression test pinning the BAM/CRAM relationship classifier invariant that a completed-bundle `VERIFIED_SEQUENCE_IDENTITY` binding must be backed by the alignment record's own `@SQ M5`.
