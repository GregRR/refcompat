...
represents a completed empty-BED analysis before Slice 3 adds BED inspection:
the report carries the explicit `bed3` layout as a generic resource observation,
contains no fabricated feature requirements, and is therefore scientifically
indeterminate rather than incompatible. The same payload is rejected by exact
schema 2.0.0 because that retained schema's closed resource-kind enum predates
BED support.
