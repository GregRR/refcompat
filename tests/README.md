...
- Milestone 3 VCF binding/revalidation tests cover valid `##contig` MD5 alias evidence with explicit declared-metadata provenance, mandatory identity projection for valid used-contig MD5 declarations, rejection of declared identity as candidate reference evidence, same-name MD5 contradiction vetoes, no-name-heuristic behavior, full-anchor duplicate-content ambiguity, anchor-scope exclusion, declared-length contradictions, conservative cross-name MD5/length inconsistency handling, many-local-names-to-one-anchor binding, binding-aware REF matches/mismatches, stale-validation rejection, binding trace determinism, projection-level `VERIFIED_ALIAS`, and whole-bundle re-derivation of the same binding.

- Milestone 4 BAM/CRAM header-observation tests cover ordered `@SQ` metadata, `@HD` ordering claims, `@PG` provenance, extension-tag tolerance, textual/binary reference-dictionary reconciliation, empty sequence dictionaries, declared-format/provider mismatches, provider-shape failures, provider closure, real BAM parsing, and CRAM header inspection without reference content available.
- Milestone 4 alignment contract-projection tests cover mandatory `SN` presence, `LN` length, and declared-M5 identity requirements; empty dictionaries; explicit resource scope; deterministic IDs; same-name exact matches and hard length/M5 contradictions; verified cross-name M5 satisfaction; `AN`-only unresolved behavior; and conservative M5/LN contradiction handling.
- Milestone 4 alignment binding tests cover declared-metadata provenance, BAM/CRAM parity, full-anchor duplicate-content ambiguity, anchor-scope exclusion, incomplete anchor M5 coverage, length contradictions, exact-name bypass, stronger identity overriding misleading string equality, many-local-names-to-one-anchor binding, and anchor-resource cross-wiring rejection.

- Milestone 4 relationship tests keep BAM/CRAM header interpretation descriptive and multidimensional: exact M5-backed identity, verified naming-only differences, reorder, subset, M5-distinct superset/overlap, content and length conflicts, missing-M5 uncertainty, scope exclusions, non-bijective bindings, and incomplete-anchor MD5 negative controls are each pinned independently.
- Milestone 4 CRAM offline-reference tests pin the deterministic boundary between reference-independent header reasoning and future record decoding: exact-name M5-verified full/subset dictionaries may use only the explicitly selected readable FASTA anchor, while cross-name addressing, missing/conflicting M5, length conflict, extra references, unreadable anchors, and header `UR` fallback all defer reference-dependent decoding.
