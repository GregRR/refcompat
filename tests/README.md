...
- Milestone 3 VCF binding/revalidation tests cover valid `##contig` MD5 alias evidence, mandatory identity projection for valid used-contig MD5 declarations, same-name MD5 contradiction vetoes, no-name-heuristic behavior, full-anchor duplicate-content ambiguity, anchor-scope exclusion, declared-length contradictions, binding-aware REF matches/mismatches, stale-validation rejection, binding trace determinism, projection-level `VERIFIED_ALIAS`, and whole-bundle re-derivation of the same binding.
