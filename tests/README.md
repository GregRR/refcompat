...
- Milestone 4 alignment contract-projection tests cover mandatory `SN` presence, `LN` length, and declared-M5 identity requirements; empty dictionaries; explicit resource scope; deterministic IDs; same-name exact matches and hard length/M5 contradictions; and deliberate deferral of `AN`/cross-name M5 binding.
