...
- explicit `Is_circular=true` landmark evidence needed to interpret circular-origin coordinates; because this attribute changes compatibility interpretation, accept it only once with the exact literal value `true`, rejecting duplicate, contradictory, comma-valued, empty, bare, or non-`true` forms as invalid input.
