...
Milestone 5 applies the same scalable pair-derived pattern to annotation coordinates. `CoordinateBoundsRequirement` is resource-level rather than one requirement per feature, names the selected anchor, and carries the in-scope feature count. Its anchor-owned `CoordinateBoundsValidationCapability` partitions those feature statements into representable, conflicting, and unresolved counts for the same subject resource. Format-specific interpretation happens before projection: GFF3 circular-origin features that are valid under the standard representation count as representable, while unresolved sequence names or uncertain circular cases remain unresolved. The annotation-specific result retains local feature/seqid detail so the generic model stays compact. A proven coordinate conflict is Tier-B structural contradiction evidence; coordinate agreement does not become sequence-content identity evidence.
